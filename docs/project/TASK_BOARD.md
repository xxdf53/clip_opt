...
| T30 | Legacy evaluation consolidation and code hygiene | review, T30.1 completed | legacy evaluation scripts, documentation entry points, and behavior-preserving hygiene outside training logic | `docs/project/T30_CODE_CLEANUP_AUDIT.md` | Remove only proven superseded code, preserve dataset protocol coverage, and pass the existing test suite without changing training or inference behavior |
...
- 2026-08-21: EXP-D20-015 completed the seed-123 fake-only weight-0.5 control. It reaches raw/calibrated Macro ACC 95.20%/88.64% versus 90.31%/88.74% for symmetric EXP-D20-014. The calibrated difference is negligible, so close the hard-real branch as an ablation and run fake-only weight 0.5 at seeds 42 and 2024 before any method promotion.
- 2026-08-21: EXP-D20-016 and EXP-D20-017 pre-register fake-only HFR weight 0.5 at seeds 42 and 2024 using the same `ds271828` manifest and all other EXP-D20-015 settings. Record raw and validation-fixed results separately before changing the active D20 decision.
- 2026-08-21: T30 read-only cleanup audit identifies `scripts/test_model.py` and `scripts/test_chameleon.py` as superseded deletion candidates. `scripts/test_sd.py` and `scripts/test_vqdm.py` remain conditional because they adapt `nature/ai` layouts not accepted by the unified evaluator. No code deletion or refactor is authorized until the user confirms the implementation phase.
- 2026-08-21: T30 file audit classifies only `.pytest_cache/` and source-tree `__pycache__/` directories as unconditional cleanup candidates. Generated plots and `tmp/pdfs` renders need user confirmation; models, data, canonical records, paper evidence, and tool state are protected. Do not use bulk `git clean` because important project files are currently untracked.
- 2026-08-21: The user confirmed T30.1 deletion of proven useless code and rebuildable caches. T30.1 is active; conditional research artifacts and dataset-layout adapters remain protected.
- 2026-08-21: T30.1 completed: removed `scripts/test_model.py`, `scripts/test_chameleon.py`, `.pytest_cache/`, and six source-tree `__pycache__/` directories; cleaned unused imports and corrected README entry points. All 94 tests pass in the `c2pclip` Conda environment, syntax and diff checks pass, and replacement official/inference CLIs load. T30 is in review; T30.2/T30.3 remain unconfirmed.
