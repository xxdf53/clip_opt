...
- 2026-08-20: Revision `f571bf5d90b0b5e679e0e11704f5338c79381db9` adds a default-off hard-real objective for EXP-D20-014. The seed-123 ablation splits HFR's auxiliary coefficient equally between the lowest-logit 25% fake samples and highest-logit 25% real samples. This is a budget-matched diagnostic of class asymmetry, not a new active method.
- 2026-08-21: EXP-D20-014 seed 123 reports raw Macro ACC 90.31% and validation-fixed Macro ACC 88.74%, AP 99.15%, AUROC 99.34%, ECE 11.33%, and Brier 0.0911. It improves the calibrated seed-123 baseline by 2.66 pp ACC while retaining ranking gains, but simultaneously halves hard-fake weight and adds hard-real. EXP-D20-015 is pre-registered as the required fake-only weight-0.5 control before assigning causality or expanding seeds.
- 2026-08-21: EXP-D20-015 completed the fake-only weight-0.5 control at seed 123. Raw Macro ACC is 95.20% with Real/Fake ACC 96.96%/93.45%; validation-fixed Macro ACC is 88.64%, AP 99.08%, AUROC 99.30%, ECE 11.39%, and Brier 0.0918. Relative to symmetric EXP-D20-014, raw ACC improves 4.89 pp but calibrated ACC differs by only -0.10 pp, with AP/AUROC -0.07/-0.04 pp and nearly identical ECE/Brier. Hard-real therefore shows no measurable post-calibration benefit in this seed and should not be added to the active HFR method; repeat the simpler fake-only half-weight arm at seeds 42 and 2024 before considering promotion.
- 2026-08-21: A read-only code cleanup audit is registered as T30. `scripts/test_model.py` and `scripts/test_chameleon.py` have no repository consumers and are functionally covered by the unified official evaluator for `0_real/1_fake` data. `scripts/test_sd.py` and `scripts/test_vqdm.py` are duplicated, hard-coded Windows adapters with destructive temporary-directory recreation, but they cannot be removed until `nature/ai` layout support is migrated and tested. Training code and the user-owned `options/train_options.py` remain outside T30 scope.
- 2026-08-21: Local T30 validation is environment-blocked before project test collection: `C:\Users\xxf\anaconda3\python.exe` imports `numpy` as an incomplete namespace package with no `__file__`, version, or `ndarray`, causing Torch import to fail. This is not recorded as a project-code test failure; T30 implementation must rerun tests in a healthy environment.
- 2026-08-21: The T30 non-code audit finds only `.pytest_cache/` and six source-tree `__pycache__/` directories (158 `.pyc` files, 850,934 bytes) to be unconditionally rebuildable deletion candidates. Root logit plots and rendered pages under `tmp/pdfs/` need explicit confirmation because they may be research artifacts. Checkpoints, datasets, captions, project records, paper handoffs, tool configuration, and active scripts must be preserved; bulk `git clean` is unsafe for this worktree.
- 2026-08-21: User-confirmed T30.1 cleanup removed two superseded official-evaluation scripts and all audited Python/pytest caches, then corrected README entry paths and removed behavior-neutral unused imports. The healthy `c2pclip` environment passes all 94 tests; the official evaluator and module-form inference CLI help commands pass. `scripts/draw_tsne_kmean.py --help` remains environment-blocked by the pre-existing optional `MulticoreTSNE` dependency. No training, model, checkpoint, dataset, metric, threshold, or calibration behavior changed.
