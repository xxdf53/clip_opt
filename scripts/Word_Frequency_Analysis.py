    ...
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            if file.lower().endswith(('.txt')):
                ...
                if os.sep + '0_real' + os.sep in  abspath_tmp: image_paths_real.append(abspath_tmp)
                if os.sep + '1_fake' + os.sep in  abspath_tmp: image_paths_fake.append(abspath_tmp)
...
if __name__ == '__main__':
    ...
    opt.root_path = opt.root_path.rstrip(os.sep).rstrip('/')
    ...
    plt.bar(x - width/2, counts_real, width, label=f'{" ".join(opt.root_path.split(os.sep)[-2:])} real')
    plt.bar(x + width/2, counts_fake, width, label=f'{" ".join(opt.root_path.split(os.sep)[-2:])} fake')
